...
local function main()
	...
	Explorer.StartDrag = function(offX,offY,inputType)
		if Explorer.Dragging then return end
		if inputType == "Touch" and not (Settings.Explorer and Settings.Explorer.TouchDragEnabled == true) then return end
	...
	Explorer.NewListEntry = function(index)
		...
		local rowHeight = Main.IsMobile() and Main.GetMobileMetrics().RowHeight or 20
		newEntry.Position = UDim2.new(0,0,0,rowHeight*(index-1))
		...
		newEntry.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				...
				local explorerSettings = Settings.Explorer or {}
				local touchDragEnabled = explorerSettings.TouchDragEnabled == true
				local touchHoldReady = not isTouchDrag
				local touchCancelled = false
				local touchHoldToken

				if isTouchDrag then
					startX = input.Position.X
					startY = input.Position.Y
					touchHoldToken = {}
					local holdToken = touchHoldToken
					task.delay(explorerSettings.TouchDragHoldTime or 0.35, function()
						if touchHoldToken == holdToken and not touchCancelled then
							touchHoldReady = true
						end
					end)
				else
				...
				mouseEvent = service.UserInputService.InputChanged:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
						...
						if input.UserInputType == Enum.UserInputType.Touch then
							if not isTouchDrag then return end
							currentX = input.Position.X
							currentY = input.Position.Y
						else
							if isTouchDrag then return end
							currentX = mouse.X
							currentY = mouse.Y
						end
						...
						if isTouchDrag then
							local scrollCancelThreshold = explorerSettings.TouchScrollCancelThreshold or 10
							local touchDragMoveThreshold = explorerSettings.TouchDragMoveThreshold or 18
							local verticalScroll = math.abs(deltaY) > scrollCancelThreshold and math.abs(deltaY) >= math.abs(deltaX)
							if not touchDragEnabled or verticalScroll then
								touchCancelled = true
								touchHoldToken = nil
								mouseEvent:Disconnect()
								return
							...
							if not touchHoldReady or dist < touchDragMoveThreshold then return end
							releaseEvent:Disconnect()
							mouseEvent:Disconnect()
							isRenaming = false
							Explorer.StartDrag(listOffsetX, listOffsetY, "Touch")
						elseif dist > 5 then
							releaseEvent:Disconnect()
							mouseEvent:Disconnect()
							isRenaming = false
	...
	Explorer.Refresh = function()
		local rowHeight = Main.IsMobile() and Main.GetMobileMetrics().RowHeight or 20
		local maxNodes = math.max(math.ceil((treeFrame.AbsoluteSize.Y) / rowHeight), 0)	
		...
		for i = 1,maxNodes do
			...
			if node then
				...
				entry.Size = UDim2.new(0,Explorer.ViewWidth,0,rowHeight)
...
local function main()
	...
	Lib.ApplyModernStroke = function(gui, color, transparency)
		if not gui then return end
		local stroke = gui:FindFirstChild("ModernStroke")
		if not stroke then
		...
		stroke.Color = color or Settings.Theme.Outline2
		stroke.Transparency = transparency or 0.55
		stroke.Thickness = 1.15
		return stroke
	end

	Lib.ApplyModernFrameStyle = function(gui, radius, strokeColor, strokeTransparency)
		if not gui then return end
		gui.BorderSizePixel = 0
		local corner = gui:FindFirstChild("ModernCorner")
		if not corner then
			for _, child in pairs(gui:GetChildren()) do
		...
		Lib.ApplyModernStroke(gui, strokeColor or Settings.Theme.Outline2, strokeTransparency or 0.65)
		return gui
	end
	...
	Lib.Window = (function()
		...
		local createGui = function(self)
			local mobileMetrics = Main.GetMobileMetrics()
			local topBarHeight = mobileMetrics.TitleBarHeight
			local isMobile = Main.IsMobile()
			local controlSize = isMobile and mobileMetrics.MinTapSize or 16
			local controlInset = isMobile and 4 or 2
			local contentTop = isMobile and topBarHeight or 20
			local resizeHandle = isMobile and mobileMetrics.ResizeHandle or 5
			local gui = create({
				...
				--[[background mod set to 0.05]]	{3,"Frame",{BackgroundColor3=Color3.new(0.17647059261799,0.17647059261799,0.17647059261799),BorderSizePixel=0,Name="Content",Parent={2},Position=UDim2.new(0,0,0,contentTop),Size=UDim2.new(1,0,1,-contentTop),ClipsDescendants=true}},
				{4,"Frame",{BackgroundColor3=Color3.fromRGB(33,33,33),BorderSizePixel=0,Name="Line",Parent={3},Size=UDim2.new(1,0,0,1),}},
				{5,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.20392157137394,0.20392157137394,0.20392157137394),BorderSizePixel=0,Name="TopBar",Parent={2},Size=UDim2.new(1,0,0,topBarHeight),Text = ""}},
				{6,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Title",Parent={5},Position=UDim2.new(0,8,0,0),Size=UDim2.new(1,-(controlSize*2+18),0,topBarHeight),Text="Window",TextColor3=Color3.new(1,1,1),TextSize=isMobile and 16 or 14,TextXAlignment=0}},
				{7,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.12549020349979,0.12549020349979,0.12549020349979),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="Close",Parent={5},Position=UDim2.new(1,-controlSize-controlInset,0,controlInset),Size=UDim2.new(0,controlSize,0,controlSize),Text="",TextColor3=Color3.new(1,1,1),TextSize=14,}},
				{8,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Image="rbxassetid://5054663650",Parent={7},Position=UDim2.new(0.5,-5,0.5,-5),Size=UDim2.new(0,10,0,10),}},
				{9,"UICorner",{CornerRadius=UDim.new(0,4),Parent={7},}},
				--[[lol mod]]	{9,"UICorner",{CornerRadius=UDim.new(0,4),Parent={2},}},
				{10,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.12549020349979,0.12549020349979,0.12549020349979),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="Minimize",Parent={5},Position=UDim2.new(1,-controlSize*2-controlInset,0,controlInset),Size=UDim2.new(0,controlSize,0,controlSize),Text="",TextColor3=Color3.new(1,1,1),TextSize=14,}},
				{11,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Image="rbxassetid://5034768003",Parent={10},Position=UDim2.new(0.5,-5,0.5,-5),Size=UDim2.new(0,10,0,10),}},
				...
				{15,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.27450981736183,0.27450981736183,0.27450981736183),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="North",Parent={14},Position=UDim2.new(0,resizeHandle,0,0),Size=UDim2.new(1,-resizeHandle*2,0,resizeHandle),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,}},
				{16,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.27450981736183,0.27450981736183,0.27450981736183),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="South",Parent={14},Position=UDim2.new(0,resizeHandle,1,-resizeHandle),Size=UDim2.new(1,-resizeHandle*2,0,resizeHandle),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,}},
				{17,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.27450981736183,0.27450981736183,0.27450981736183),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="NorthEast",Parent={14},Position=UDim2.new(1,-resizeHandle,0,0),Size=UDim2.new(0,resizeHandle,0,resizeHandle),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,}},
				{18,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.27450981736183,0.27450981736183,0.27450981736183),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="East",Parent={14},Position=UDim2.new(1,-resizeHandle,0,resizeHandle),Size=UDim2.new(0,resizeHandle,1,-resizeHandle*2),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,}},
			...
			guiResizeControls.Visible = self.Resizable and self.ResizableInternal and not Main.IsMobile()
			guiTopBar.BackgroundColor3 = Settings.Theme.Main2
			self.GuiElems.Content.BackgroundColor3 = Settings.Theme.Main1
			self.GuiElems.Line.BackgroundColor3 = Settings.Theme.Accent
			self.GuiElems.Outlines.ImageColor3 = Settings.Theme.AccentSoft
			self.GuiElems.Outlines.ImageTransparency = 0.25
			Lib.ApplyModernFrameStyle(guiMain, 6, Settings.Theme.AccentSoft, 0.62)
			Lib.ApplyModernFrameStyle(self.GuiElems.Content, 6, Settings.Theme.Outline1, 0.78)
			Lib.ApplyModernStroke(guiTopBar, Settings.Theme.Accent, 0.78)
			Lib.ApplyModernFrameStyle(guiTopBar.Close, 5, Settings.Theme.Outline2, 0.68)
			Lib.ApplyModernFrameStyle(guiTopBar.Minimize, 5, Settings.Theme.Outline2, 0.68)
			...
			if Settings.Window.TitleOnMiddle then
				...
				self.GuiElems.Title.Size = UDim2.new(1,-(controlSize*2+18),0,topBarHeight)
			end
		...
		funcs.SetMinimized = function(self,set,mode)
			local collapsedHeight = Main.IsMobile() and Main.GetMobileMetrics().TitleBarHeight or 20
			...
			if mode == 1 or mode == 2 then
				self:StopTweens()
				if mode == 1 then
					self.GuiElems.Main:TweenSize(UDim2.new(0,self.SizeX,0,newVal and collapsedHeight or self.SizeY),Enum.EasingDirection.Out,Enum.EasingStyle.Quart,0.25,true)
				else
					local maxY = sidesGui.AbsoluteSize.Y
					local newPos = UDim2.new(0,self.PosX,0,newVal and math.min(maxY-collapsedHeight,self.PosY + self.SizeY - collapsedHeight) or math.max(0,self.PosY - self.SizeY + collapsedHeight))
		...
		funcs.Resize = function(self,sizeX,sizeY)
			...
			Main.ClampWindowToViewport(self)
		end
		...
		funcs.SetResizable = function(self,val)
			self.Resizable = val
			self.GuiElems.ResizeControls.Visible = self.Resizable and self.ResizableInternal and not Main.IsMobile()
		end
		...
		funcs.AlignTo = function(self,side,pos,size,silent)
			if Main.IsMobile() then
				if self.Closed then
					static.ShowWindow(self,{Silent = silent})
				else
					self:SetAligned(false)
					Main.ShowMobileWindow(self)
				end
				return
			end
		...
		funcs.Close = function(self)
			...
			if not self.Aligned then
				...
				self:DoTween(self.GuiElems.Main,ti,{Size = UDim2.new(0,self.SizeX,0,collapsedHeight)})
		...
		static.ShowWindow = function(window,data)
			...
			local isMobile = Main.IsMobile()
			if isMobile then align = nil end
			...
			if not window.Closed then
				...
				if isMobile then Main.ShowMobileWindow(window) end
				return
			end
			...
			if align then
				...
				if align == nil and window.ClosedSide then -- Regular open
					...
					-- TODO: make better
					local collapsedHeight = isMobile and Main.GetMobileMetrics().TitleBarHeight or 20
					window.GuiElems.Main.Size = UDim2.new(0,window.SizeX,0,collapsedHeight)
					local ti = TweenInfo.new(0.2,Enum.EasingStyle.Quad,Enum.EasingDirection.Out)
					window:StopTweens()
	...
	Lib.ContextMenu = (function()
		...
		local function createGui(self)
			...
			contextGui.Main.BackgroundColor3 = Settings.Theme.AccentSoft
			contextGui.Main.Container.BackgroundColor3 = Settings.Theme.Menu
			self.GuiElems.SearchFrame.BackgroundColor3 = Settings.Theme.Main2
			self.GuiElems.SearchFrame.SearchContainer.BackgroundColor3 = Settings.Theme.TextBox
			self.GuiElems.SearchFrame.Line.BackgroundColor3 = Settings.Theme.AccentSoft
			Lib.ApplyModernFrameStyle(contextGui.Main, 6, Settings.Theme.AccentSoft, 0.5)
			Lib.ApplyModernFrameStyle(contextGui.Main.Container, 5, Settings.Theme.Outline1, 0.72)
			Lib.ApplyModernFrameStyle(self.GuiElems.SearchFrame.SearchContainer, 4, Settings.Theme.Outline2, 0.65)
		...
		funcs.Show = function(self,x,y)
			...
			local mobileMetrics = Main.IsMobile() and Main.GetMobileMetrics()
			if mobileMetrics then
				self.Width = math.min(math.max(self.Width, 220), math.max(220, maxX - mobileMetrics.Padding * 2))
				local minMobileX = mobileMetrics.Padding
				local maxMobileX = maxX - self.Width - mobileMetrics.Padding
				if maxMobileX >= minMobileX then
					x = math.clamp(x, mobileMetrics.Padding, maxX - self.Width - mobileMetrics.Padding)
				else
					x = minMobileX
				end
			end
			...
			if mobileMetrics then
				local fullContentSize = elems.List.UIListLayout.AbsoluteContentSize.Y + 6
				local maxMobileHeight = math.max(120, maxY - mobileMetrics.BottomNavHeight - mobileMetrics.Padding * 2)
				local shouldBottomSheet = mobileMetrics and (self.MobileBottomSheet or fullContentSize > maxMobileHeight)
				toSize = math.min(fullContentSize, maxMobileHeight)
				elems.List.CanvasSize = fullContentSize > toSize and UDim2.new(0,0,0,fullContentSize-6) or UDim2.new(0,0,0,0)
				if shouldBottomSheet then
					local bottomY = maxY - mobileMetrics.BottomNavHeight - toSize - mobileMetrics.Padding
					y = math.max(mobileMetrics.Padding, bottomY)
				else
					local minMobileY = mobileMetrics.Padding
					local maxMobileY = maxY - mobileMetrics.BottomNavHeight - toSize - mobileMetrics.Padding
					if maxMobileY >= minMobileY then
						y = math.clamp(y, mobileMetrics.Padding, maxY - mobileMetrics.BottomNavHeight - toSize - mobileMetrics.Padding)
					else
						y = minMobileY
					end
				end
	...
	Lib.Frame = (function()
		...
		local function new()
			...
			Lib.ApplyModernFrameStyle(fr, 5, Settings.Theme.Outline1, 0.72)
	...
	Lib.Button = (function()
		...
		local function new()
			...
			Lib.ApplyModernFrameStyle(b, 5, Settings.Theme.Outline2, 0.58)
...
local function main()
	...
	ModelViewer.Init = function()
		...
		RunService.RenderStepped:Connect(function(dt)
			if not Settings.Performance.ModelViewerRenderWhenHidden then
				if not (window and window:IsContentVisible()) then return end
			end
...
local function main()
	...
	Properties.Refresh = function()
		local rowHeight = Main.IsMobile() and Main.GetMobileMetrics().RowHeight or 22
		local maxEntries = math.max(math.ceil((propsFrame.AbsoluteSize.Y) / (rowHeight + 1)),0)
		local maxX = propsFrame.AbsoluteSize.X
		...
		for i = 1,maxEntries do
			...
			if prop then
				...
				entry.Size = UDim2.new(scaleType == 0 and 0 or 1, scaleType == 0 and Properties.ViewWidth + valueWidth or 0,0,rowHeight)
	...
	Properties.InitSearch = function()
		...
		searchBox:GetPropertyChangedSignal("Text"):Connect(function()
			...
			local currentVersion = searchVersion
			local function applySearch()
				if currentVersion ~= searchVersion then return end
				Properties.SearchText = searchBox.Text
				Properties.Update()
				Properties.Refresh()
			end
			if Settings.Performance.SafeMode then
				task.delay(Settings.Performance.SearchDebounce or 0.12, applySearch)
			else
				applySearch()
			end
		end)
...
local function main()
	...
	SettingsWindow.Init = function()
		...
		local mobileMode = AddDropdown("Mobile Mode", {"Auto", "On", "Off"}, Settings.Mobile.Mode, false, 75)
		mobileMode.OnSelect:Connect(function()
			Settings.Mobile.Mode = mobileMode.Selected
		end)
		
		local mobileScale = AddTextbox("Mobile UI Scale", tostring(Settings.Mobile.UIScale), 35)
		mobileScale.FocusLost:Connect(function()
			local scale = tonumber(mobileScale.Text)
			if scale then
				Settings.Mobile.UIScale = math.clamp(scale, 0.75, 1.5)
				mobileScale.Text = tostring(Settings.Mobile.UIScale)
			else
				mobileScale.Text = tostring(Settings.Mobile.UIScale)
			end
		end)
		
		local minTapSize = AddTextbox("Min Tap Size", tostring(Settings.Mobile.MinTapSize), 35)
		minTapSize.FocusLost:Connect(function()
			local size = tonumber(minTapSize.Text)
			if size then
				Settings.Mobile.MinTapSize = math.clamp(size, 30, 56)
				minTapSize.Text = tostring(Settings.Mobile.MinTapSize)
			else
				minTapSize.Text = tostring(Settings.Mobile.MinTapSize)
			end
		end)
		
		local singlePanel = AddCheckbox("Mobile Single Panel", Settings.Mobile.SinglePanel)
		singlePanel.OnInput:Connect(function()
			Settings.Mobile.SinglePanel = singlePanel.Toggled
		end)
		
		local actionPalette = AddCheckbox("Mobile Action Palette", Settings.Mobile.ActionPalette)
		actionPalette.OnInput:Connect(function()
			Settings.Mobile.ActionPalette = actionPalette.Toggled
		end)
		
		AddSeperator("Performance")
		
		local safeMode = AddCheckbox("Safe Performance Mode", Settings.Performance.SafeMode)
		safeMode.OnInput:Connect(function()
			Settings.Performance.SafeMode = safeMode.Toggled
		end)
		
		local adonisOptIn = AddCheckbox("Auto Run Adonis Bypass", Settings.Performance.AutoRunAdonisBypass)
		adonisOptIn.OnInput:Connect(function()
			Settings.Performance.AutoRunAdonisBypass = adonisOptIn.Toggled
		end)
		
		local remoteSpyAutoHook = AddCheckbox("Remote Spy Auto Hook", Settings.Performance.RemoteSpyAutoHook)
		remoteSpyAutoHook.OnInput:Connect(function()
			Settings.Performance.RemoteSpyAutoHook = remoteSpyAutoHook.Toggled
		end)
		
		local hiddenModelRender = AddCheckbox("Render 3D Viewer When Hidden", Settings.Performance.ModelViewerRenderWhenHidden)
		hiddenModelRender.OnInput:Connect(function()
			Settings.Performance.ModelViewerRenderWhenHidden = hiddenModelRender.Toggled
		end)
...
DefaultSettings = (function()
	...
	return {
		...
		Theme = {
			...
			Main1 = rgb(19,24,31),
			Main2 = rgb(13,18,25),
			Outline1 = rgb(28,119,170), -- Mainly frames
			Outline2 = rgb(44,70,88), -- Mainly button
			Outline3 = rgb(12,32,43), -- Mainly textbox
			TextBox = rgb(10,15,22),
			Menu = rgb(11,17,24),
			ListSelection = rgb(0,126,190),
			Button = rgb(24,34,45),
			ButtonHover = rgb(32,52,66),
			ButtonPress = rgb(7,25,34),
			Highlight = rgb(0,210,255),
			Accent = rgb(0,210,255),
			AccentSoft = rgb(28,119,170),
			AccentGlow = rgb(75,225,255),
			SurfaceGlass = rgb(16,22,30),
			Text = rgb(232,246,255),
			PlaceholderText = rgb(106,132,148),
			Important = rgb(255,77,109),
			ExplorerIconMap = "",
			MiscIconMap = "",
			Syntax = {
...
-- Vars
local function CloneDefaultSettings(tbl)
	if type(tbl) ~= "table" then return tbl end
	local newTbl = {}
	for k, v in pairs(tbl) do
		newTbl[k] = CloneDefaultSettings(v)
	end
	return newTbl
end

local Settings = DefaultSettings and CloneDefaultSettings(DefaultSettings) or {}
local Apps = {}
local env = {}
...
Main = (function()
	...
	Main.IsMobile = function()
		...
		local mode = mobileSettings.Mode or "Auto"
		if mode == "On" then return true end
		if mode == "Off" then return false end
		return service.UserInputService.TouchEnabled == true
	end
	
	Main.GetViewportSize = function()
	...
	Main.GetMobileMetrics = function()
		local mobileSettings = Settings.Mobile or {}
		local function openMax(value, fallback)
			local num = tonumber(value) or fallback
			if num <= 0 then return 99999 end
			return math.floor(math.max(1, num))
		end
		local scale = math.clamp(tonumber(mobileSettings.UIScale) or 0.74, 0.55, 1.6)
		local tap = math.floor(math.clamp(tonumber(mobileSettings.MinTapSize) or 28, 20, 64))
		local rowHeight = math.floor(math.clamp(tonumber(mobileSettings.RowHeight) or 20, 16, 48))
		local navHeight = mobileSettings.BottomNavigation == false and 0 or math.floor(math.max(34, tap + 8))
		local launcherSize = math.floor(math.clamp(tonumber(mobileSettings.LauncherSize) or 36, 22, 72))
		local launcherPosition = mobileSettings.LauncherPosition or {}
		return {
			Scale = scale,
			MinTapSize = tap,
			RowHeight = rowHeight,
			TitleBarHeight = math.max(rowHeight, math.floor(28 * scale)),
			BottomNavHeight = navHeight,
			Padding = math.max(5, math.floor(6 * scale)),
			ResizeHandle = math.floor(8 * scale),
			ResizeHandleSize = math.floor(math.clamp(tonumber(mobileSettings.ResizeHandleSize) or 22, 16, 48)),
			...
			PanelWidthLandscape = math.clamp(tonumber(mobileSettings.PanelWidthLandscape) or 0.34, 0.20, 1),
			PanelWidthPortrait = math.clamp(tonumber(mobileSettings.PanelWidthPortrait) or 0.68, 0.30, 1),
			PanelHeightLandscape = math.clamp(tonumber(mobileSettings.PanelHeightLandscape) or 0.36, 0.20, 1),
			PanelHeightPortrait = math.clamp(tonumber(mobileSettings.PanelHeightPortrait) or 0.30, 0.20, 1),
			...
			MaxPanelWidth = openMax(mobileSettings.MaxPanelWidth, 99999),
			MaxPanelHeight = openMax(mobileSettings.MaxPanelHeight, 99999),
			...
			PaletteMaxHeight = math.floor(math.clamp(tonumber(mobileSettings.PaletteMaxHeight) or 260, 150, 380)),
			PaletteHeight = math.clamp(tonumber(mobileSettings.PaletteHeight) or 0.36, 0.24, 0.52),
			...
			LauncherDragThreshold = math.max(5, math.floor(6 * scale))
		}
	end

	Main.GetMobileLayoutBudget = function(viewport, metrics)
		metrics = metrics or Main.GetMobileMetrics()
		viewport = viewport or Main.GetViewportSize()
		local margin = metrics.Padding
		local usableWidth = math.max(metrics.PanelMinWidth, viewport.X - margin * 2)
		local usableHeight = math.max(metrics.PanelMinHeight, viewport.Y - margin * 2)
		...
		local resizeMaxWidth = math.min(usableWidth, metrics.MaxPanelWidth)
		local resizeMaxHeight = math.min(usableHeight, metrics.MaxPanelHeight)
		local panelWidth = math.floor(math.min(resizeMaxWidth, viewport.X * panelWidthScale))
		local panelHeight = math.floor(math.min(usableHeight * metrics.DefaultPanelHeight, viewport.Y * panelHeightScale, resizeMaxHeight))
		local menuWidthScale = isLandscape and 0.42 or 0.60
		local menuWidth = math.floor(math.min(viewport.X * menuWidthScale, metrics.MenuMaxWidth))
		local menuHeight = math.floor(math.min(viewport.Y * metrics.CompactPanelHeight, metrics.MenuMaxHeight))
		local paletteWidth = math.floor(math.min(metrics.PaletteMaxWidth, viewport.X - margin * 2))
		local paletteHeight = math.floor(math.min(viewport.Y * metrics.PaletteHeight, metrics.PaletteMaxHeight))
		return {
			PanelWidth = math.min(math.max(metrics.PanelMinWidth, panelWidth), usableWidth),
			PanelHeight = math.min(math.max(metrics.PanelMinHeight, panelHeight), usableHeight),
			ResizeMaxWidth = math.max(metrics.PanelMinWidth, resizeMaxWidth),
			ResizeMaxHeight = math.max(metrics.PanelMinHeight, resizeMaxHeight),
			MenuWidth = math.max(140, menuWidth),
			MenuHeight = math.max(110, menuHeight),
			PaletteWidth = math.max(180, paletteWidth),
			PaletteHeight = math.max(140, paletteHeight)
		}
	end
	
	Main.ClampWindowToViewport = function(window)
		if not Main.IsMobile() or not window or not window.GuiElems or not window.GuiElems.Main then return end
		...
		local margin = metrics.Padding
		local hasOverride = Main.ApplyWindowOverride(window)
		local targetWidth = (hasOverride or window.MobileSizedOnce) and window.SizeX or layoutBudget.PanelWidth
		local targetHeight = (hasOverride or window.MobileSizedOnce) and window.SizeY or layoutBudget.PanelHeight
		window.SizeX = math.clamp(targetWidth, metrics.PanelMinWidth, layoutBudget.ResizeMaxWidth)
		window.SizeY = math.clamp(targetHeight, metrics.PanelMinHeight, layoutBudget.ResizeMaxHeight)
		window.MobileSizedOnce = true
	...
	Main.AttachMobileWindowResize = function(window)
		...
		handle.InputBegan:Connect(function(input)
			...
			moveCon = service.UserInputService.InputChanged:Connect(function(moveInput)
				...
				window.SizeX = math.clamp(startSize.X + delta.X, metrics.PanelMinWidth, layoutBudget.ResizeMaxWidth)
				window.SizeY = math.clamp(startSize.Y + delta.Y, metrics.PanelMinHeight, layoutBudget.ResizeMaxHeight)
	...
	Main.EnsureMobileResizeHandles = function()
	...
	Main.ShowMobileWindow = function(window)
		if not Main.IsMobile() or not window then return end
		local mobileSettings = Settings.Mobile or {}
		if mobileSettings.SinglePanel ~= false then
			for _, other in pairs(Main.MobileWindows) do
				if other ~= window and other.IsVisible and other:IsVisible() and other.Hide then
					other:Hide()
				end
			end
		end
		Main.MobileActiveWindow = window
		Main.ClampWindowToViewport(window)
	end
	...
	Main.ResizeMobileWindowByPreset = function(preset)
		...
		window.SizeX = math.clamp(math.floor(layoutBudget.ResizeMaxWidth * scale), metrics.PanelMinWidth, layoutBudget.ResizeMaxWidth)
		window.SizeY = math.clamp(math.floor(layoutBudget.ResizeMaxHeight * scale), metrics.PanelMinHeight, layoutBudget.ResizeMaxHeight)
		Main.SaveWindowOverride(window)
		Main.ClampWindowToViewport(window)
	end

	Main.WatchMobileViewport = function()
		if Main.MobileViewportWatcherAttached or not Main.IsMobile() then return end
		Main.MobileViewportWatcherAttached = true
		local pending = false
		...
		local function watchCamera(camera)
			if not camera or Main.MobileViewportCamera == camera then return end
			if Main.MobileViewportConnection then Main.MobileViewportConnection:Disconnect() end
			Main.MobileViewportCamera = camera
			Main.MobileViewportConnection = camera:GetPropertyChangedSignal("ViewportSize"):Connect(scheduleRefresh)
		end
		...
		pcall(function()
			service.UserInputService:GetPropertyChangedSignal("ScreenOrientation"):Connect(scheduleRefresh)
		end)
	end
	...
	Main.RunInitStep = function(intro, label, progress, fn)
		if intro and intro.SetProgress then intro.SetProgress(label, progress) end
		local ok, err = xpcall(fn, debug.traceback)
		if not ok then
			Main.LastInitError = tostring(err)
			warn("Dex++ init failed at "..label..": "..Main.LastInitError)
			if intro and intro.SetProgress then intro.SetProgress("Failed: "..label, progress) end
			return false, err
		end
		return true
	end
	...
	Main.GetSaveInstance = function()
		if Main.SaveInstanceFn then return Main.SaveInstanceFn end
		if game:GetService("RunService"):IsStudio() then
			Main.SaveInstanceFn = function() error("Cannot run in Roblox Studio!") end
			return Main.SaveInstanceFn
		end
		local Params = {
		...
		Main.SaveInstanceFn = function(obj, filepath, options)
			options = options or {}
		...
		genv.saveinstance = Main.SaveInstanceFn
		return Main.SaveInstanceFn
	end
	...
	Main.ApplyMissingDefaultSettings = function(res, defaults)
		...
		local function recur(res, defaults)
			for set, val in pairs(defaults) do
				if type(val) == "table" then
		...
		if type(res) == "table" and type(defaults) == "table" then
			return recur(res, defaults)
		end
		return res
	end
	...
	Main.ApplyMobileLayoutMigration = function()
		...
		if mobileSettings.LayoutVersion ~= 5 then
			...
			mobileSettings.MaxPanelWidth = 99999
			mobileSettings.MaxPanelHeight = 99999
	...
	Main.CreateApp = function(data)
		...
		app.Main.AppName.TextColor3 = Settings.Theme.Text
		app.Main.Highlight.BackgroundColor3 = Settings.Theme.Accent
		...
		local function updateState()
			...
			if appStroke then
				appStroke.Color = data.Open and Settings.Theme.Accent or Settings.Theme.Outline2
				appStroke.Transparency = data.Open and 0.25 or 0.82
			end
		...
		Main.AppsFrame.CanvasSize = UDim2.new(0,0,0,Main.AppsContainerGrid.AbsoluteContentSize.Y + 8)
	...
	Main.GetMobileMenuSize = function()
		if Main.IsMobile() then
			...
			local viewport = Main.GetViewportSize()
			local layoutBudget = Main.GetMobileLayoutBudget(viewport, metrics)
			return UDim2.new(0, layoutBudget.MenuWidth, 0, layoutBudget.MenuHeight)
		end
	...
	Main.FilterActionPalette = function(query)
		if not Main.MainGui or not Main.MainGui.ActionPalette then return end
		query = string.lower(query or "")
		local list = Main.MainGui.ActionPalette.PaletteList
		for _, child in pairs(list:GetChildren()) do
			if child:IsA("TextButton") then child:Destroy() end
		end
		local function addRow(name, callback)
			...
			row.Name = "PaletteAction"
			row.Size = UDim2.new(1, 0, 0, Main.GetMobileMetrics().RowHeight)
			row.Text = name
			row.TextColor3 = Settings.Theme.Text
			row.TextSize = Main.IsMobile() and 13 or 16
			row.TextXAlignment = Enum.TextXAlignment.Left
			row.ZIndex = 12
			local padding = Instance.new("UIPadding", row)
			padding.PaddingLeft = UDim.new(0, 10)
			Lib.ApplyModernFrameStyle(row, 5, Settings.Theme.Outline2, 0.65)
			row.MouseButton1Click:Connect(function()
				Main.SetActionPaletteVisible(false)
				callback()
			end)
			row.Parent = list
		end
		addRow("Reset mobile layout", function() Main.ResetMobileLayout() end)
		for _, control in ipairs(Main.AppSequence) do
			addRow(control.Name, function()
				if control.Window then control.Window:Show() elseif control.Enable then control.Enable() end
			end)
		end
		list.CanvasSize = UDim2.new(0,0,0,list.UIListLayout.AbsoluteContentSize.Y + 8)
	end
	
	Main.SetActionPaletteVisible = function(force)
		if not Main.MainGui or not Main.MainGui.ActionPalette then return end
		if Settings.Mobile and Settings.Mobile.ActionPalette == false and force ~= false then return end
		local palette = Main.MainGui.ActionPalette
		local newVisible = force
		if newVisible == nil then newVisible = not palette.Visible end
		palette.Visible = newVisible
		if newVisible then
			Main.SetMainGuiOpen(false)
			Main.FilterActionPalette(palette.PaletteSearch.Text)
		end
	end
	
	Main.ToggleActionPalette = function()
		Main.SetActionPaletteVisible()
	end
	...
	Main.ApplyMobileMainGui = function()
		...
		local paletteHeight = layoutBudget.PaletteHeight
	...
	Main.CreateMainGui = function()
		local gui = create({
			...
			{22,"Frame",{BackgroundColor3=Color3.new(0.17647059261799,0.17647059261799,0.17647059261799),BorderSizePixel=0,Name="ActionPalette",Parent={1},Position=UDim2.new(0,8,0,8),Size=UDim2.new(1,-16,1,-72),Visible=false,ZIndex=10,}},
			{23,"UICorner",{CornerRadius=UDim.new(0,6),Parent={22},}},
			{24,"TextBox",{BackgroundColor3=Color3.new(0.1176470592618,0.1176470592618,0.1176470592618),BorderSizePixel=0,ClearTextOnFocus=false,Font=3,Name="PaletteSearch",Parent={22},PlaceholderColor3=Color3.new(0.39215689897537,0.39215689897537,0.39215689897537),PlaceholderText="Search apps / actions",Position=UDim2.new(0,8,0,8),Size=UDim2.new(1,-16,0,34),Text="",TextColor3=Color3.new(1,1,1),TextSize=16,TextXAlignment=0,ZIndex=11,}},
			{25,"UICorner",{CornerRadius=UDim.new(0,4),Parent={24},}},
			{26,"ScrollingFrame",{Active=true,BackgroundTransparency=1,BorderSizePixel=0,CanvasSize=UDim2.new(0,0,0,0),Name="PaletteList",Parent={22},Position=UDim2.new(0,8,0,48),ScrollBarThickness=6,Size=UDim2.new(1,-16,1,-56),ZIndex=11,}},
	...
	Main.Init = function()
		...
		-- Load other modules
		if not Main.RunInitStep(intro, "Loading Modules", 0.75, function()
		...
		-- Init other modules
		if not Main.RunInitStep(intro, "Initializing Modules", 0.8, function()
			Explorer.Init()
			Properties.Init()
			ScriptViewer.Init()
			Console.Init()
...
local function main()
	...
	spy.Init = function()
		...
		local filter_version = 0
		search_box:GetPropertyChangedSignal("Text"):Connect(function()
			filter_version += 1
			local current_version = filter_version
			local function apply_filter()
				if current_version ~= filter_version then return end
				filter_text = search_box.Text
				rebuild_list()
			end
			if Settings.Performance.SafeMode then
				task.delay(Settings.Performance.SearchDebounce or 0.12, apply_filter)
			else
				apply_filter()
			end
		end)
		...
		if Settings.Performance.RemoteSpyAutoHook then
			start()
			start_btn.BackgroundColor3 = Color3.fromRGB(28, 95, 48)
			start_btn.Text = "â–¶ active"
		end
	end
...
if Settings.Performance.AutoRunAdonisBypass then
	InitAdonisBypass()
end
